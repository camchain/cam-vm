...
namespace Cam.VM
{
    ...
    {
        private readonly IScriptTable script_table;

        private readonly IBlock table;
        ...
        public ExecutionEngine(IScriptContainer container, ICrypto crypto, IScriptTable scriptTable = null, IBlock table = null, InteropService service = null)
        {
                    ...
                    case OpCode.LOCK:
                        {
                            ...
                            uint timestamp = (uint)EvaluationStack.Pop().GetBigInteger();
                            byte[] pubkey = EvaluationStack.Pop().GetByteArray();
                            byte[] signature = EvaluationStack.Pop().GetByteArray();



                            uint currentTimestamp = table.GetTimestamp();

                            DateTime cTime = Helper.ToDateTime(timestamp);
                            DateTime blockTime = Helper.ToDateTime(currentTimestamp);

                            if (currentTimestamp < timestamp)
                            {
                                EvaluationStack.Push(false);//如果时间还没到期，则验证失败
                            }
